...
router.post('/register', async(req, res)=>{
    const [exists] = await conn.query(`SELECT usuario.nombre, usuario.password, usuario.id FROM usuario WHERE usuario.nombre= '${req.body.user}'`);
    if(exists[0]){
        return res.json({message: `El nombre de usuario ${req.body.user} ya existe.`});
    }
    const password = await bcrypt.hash(req.body.password, 10);
    const [response] = await conn.query(`INSERT INTO usuario (nombre, password) VALUES ('${req.body.user}','${password}')`);
    if(!response.insertId){
        return res.json({message: `Se presento un error ,${req.body.user} disculpanos.`});
    }
