 ...
 */
async function register_user(user, password_user) {
    let result;
	try {
	...
	try {
		...
		return "Error en registrar usuario o generar el hash de la contrasenia" + error;
	}
    try{
        result = await conn.query("SELECT usuario.nombre, usuario.id FROM usuario WHERE usuario.nombre= $1", [user]);
